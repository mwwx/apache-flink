 ...
 */
public class JdbcUpRetractTableSinkFactoryTest {
	...
	public void testJdbcCommonProperties() {
		...
		final DescriptorProperties descProperties = new DescriptorProperties(true);
		descProperties.putProperties(properties);

		final JdbcTableSource expected = JdbcTableSource.builder()
			.setOptions(options)
			.setSchema(schema)
			.setFieldMapping(SchemaValidator
				.deriveFieldMapping(
					descProperties,
	...
	public void testJdbcLookupProperties() {
		...
		properties.put("lookup.cache.strategy", "LRU");
	...
	public void testJdbcValidation() {
		...
		try {
			...
			TableFactoryService.find(StreamTableSourceFactory.class, properties)
				.createStreamTableSource(properties);
			fail("exception expected");
		} catch (NoMatchingTableFactoryException ignored) {
		}
