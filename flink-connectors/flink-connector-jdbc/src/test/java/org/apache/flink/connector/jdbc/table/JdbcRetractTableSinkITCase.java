...
/**
 * IT case for {@link JdbcRetractTableSinkITCase}.
 */
public class JdbcRetractTableSinkITCase extends AbstractTestBase {

	public static final String DB_URL = "jdbc:derby:memory:retract";
	public static final String DB_DRIVER = "org.apache.derby.jdbc.EmbeddedDriver";
	public static final String OUTPUT_TABLE1 = "retractSink";
	public static final String OUTPUT_TABLE2 = "appendSink";
	public static final String OUTPUT_TABLE3 = "batchSink";
	public static final String OUTPUT_TABLE4 = "retractSinkNull";
	...
	public void before() throws ClassNotFoundException, SQLException {
		...
		try (
			...
			stat.executeUpdate("CREATE TABLE " + OUTPUT_TABLE4 + " (" +
				"ID VARCHAR(20)," +
				"SUM_T BIGINT NOT NULL DEFAULT 0)");
	...
	public static DataStream<Tuple2<Integer, String>> get2TupleDataStream(StreamExecutionEnvironment env) {
		List<Tuple2<Integer, String>> data = new ArrayList<>();
		data.add(new Tuple2<>(1, "Hi"));
		data.add(new Tuple2<>(2, "Hello"));
		data.add(new Tuple2<>(3, "aaa"));
		data.add(new Tuple2<>(4, "Hi"));
		data.add(new Tuple2<>(5, "aaa"));
	...
	@Test
	public void testRetractNullValue() throws Exception {
		StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
		env.getConfig().enableObjectReuse();
		env.setStreamTimeCharacteristic(TimeCharacteristic.EventTime);
		StreamTableEnvironment tEnv = StreamTableEnvironment.create(env);

		Table t = tEnv.fromDataStream(get2TupleDataStream(env).assignTimestampsAndWatermarks(
		...
		TableResult tableResult = tEnv.executeSql("INSERT INTO upsertSink \n" +
			"SELECT aid as ID, SUM(total) AS SUM_T\n" +
			"FROM \n(" +
			"	SELECT (CASE WHEN id ='aaa' THEN NULL ELSE id END) aid, total from T)\n" +
			"GROUP BY aid");
		// wait to finish
		tableResult.getJobClient().get().getJobExecutionResult(Thread.currentThread().getContextClassLoader()).get();
		check(new Row[] {
				...
				Row.of("Hello", 2),
				Row.of(null, 8)
		}, DB_URL, OUTPUT_TABLE4, new String[]{"ID", "SUM_T"});
	}
